...
// incremented for `\n` characters, so on binary files this value will not
// be useful as it would in text files.
const Diff = struct {
    byte_idx: u64,
    line_idx: u64,
    a: u8,
...
fn cmp(filename_a: []const u8, filename_b: []const u8, flag_l: bool, flag_s: bool) !u8 {
    ...
    while (true) {
        ...
        // to indicate this then break the loop.  It is unspecified if a
        // diagnostic message is printed using the `-s` flag, here I've chosen
        // to print it regardless for simplicity.
        diff.a = a orelse {
    ...
    if (!flag_s) {
        for (diffs.items) |d| {
            if (flag_l) {
                ...
                try out.print("{s} {s} differ: char {d}, line {d}\n", .{ filename_a, filename_b, d.byte_idx, d.line_idx });
            }
        }
    }
